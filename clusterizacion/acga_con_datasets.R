...
#Genera el dataset
dataset <- generar_dataset(nombre_dataset);
puntos  = dataset[[1]];
red 	= dataset[[2]];
...
distancias_anteriores = matrix(Inf, nrow=poblacion, ncol=nrow(puntos));
...
for(corrida in 1:corridas){
	...
	for(generacion in 1:generaciones){
		...
		for(cromosoma in 1:poblacion){
			fitness[cromosoma] = calcular_fitness(cromosomas[cromosoma, ], promedio, k_max, puntos, distancias_anteriores, clusters_anteriores, cambios);
		}		
		...
		if(generacion%%1 == 0){
                        ...
                        nn <- apply(cromosomas,1,function(x, k){return(length(which(x[1:k]>= 0.5)))}, k=k_max)
...
x = 1:generaciones;
dev.new()
plot(x, registro_de_fitness, ylim=range(c(registro_de_fitness-registro_de_error_fitness, registro_de_fitness+registro_de_error_fitness)));
...
dev.new()
plot(x, registro_de_n, ylim=range(c(registro_de_n-registro_de_error_n, registro_de_n+registro_de_error_n)));

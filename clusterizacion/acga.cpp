...
-------------------*/
int especies = 3; //Una especie esta compuesta por toda una poblacion y las especies se van evolucionando en paralelo
int poblacion = 40;
...
int main(){
	...
	for(generacion = 0; generacion < generaciones; generacion++){
		...
		if((generacion+1) % cada_cuantas_generaciones_intercambiar_especies == 0){
			...
			for(int cromosoma = 0; cromosoma < soluciones_de_elite_a_intercambiar; cromosoma++){
				for(int especie = 0; especie < (especies-1); especie++){
					cromosomas[especie][cromosoma] = intercambiar_cromosomas[especie+1][cromosoma];
				}
				cromosomas[(especies-1)][cromosoma] = intercambiar_cromosomas[0][cromosoma];
			}

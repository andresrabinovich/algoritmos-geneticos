...
calcular_fitness <- function(cromosoma, matriz_de_distancias){
	...
	#s = matrix(0, ncol=1, nrow=ncol(cromosoma));
	#for(observacion in 1:ncol(cromosoma)){
	#	a = matrix(0, ncol=2, nrow=ncol(matriz_de_distancias));
	#	puntos <- c(1:ncol(cromosoma));
	#	puntos <- puntos[-observacion]; #Sacamos el punto que estamos viendo de los puntos a siluetear
	#	for(punto in puntos)){
	#		#Sumamos distinto 
	#		a[cromosoma[observacion], 1]
	#	}
	#}
	#s<-silhouette(cromosoma, matriz_de_distancias);
	#si<-summary(s);
	#return (si$si.summary["Mean"]); #Le sumamos 1 para que sea positiva
